...
class Net(CNN):
    def __init__(self):
        ...
        self.classifier = nn.Linear(cfg.NGM.GNN_FEAT[-1] + (1 if cfg.NGM.SK_EMB else 0), 1, bias_attr=True)
    ...
    def forward(self, data_dict, **kwargs):
        ...
        if 'images' in data_dict or 'features' in data_dict:
            ...
            dmax = (paddle.max(paddle.sum(K, axis=2, keepdim=True), axis=1, keepdim=True).values + 1e-5)
            K = K / dmax * 1000
            A = (K > 0).to(K.dtype)
            emb = paddle.ones(K.shape[0], K.shape[1], 1)
        ...
        s = paddle.reshape(v,(v.shape[0], tgt_len, -1)).transpose((0, 2, 1))
        ...
        if self.training or cfg.NGM.GUMBEL_SK <= 0:
            ...
            repeat = lambda x, rep_num=gumbel_sample_num: paddle.to_tensor(np.repeat(x, rep_num, axis=0))
            if not self.training:
                ss_gumbel = hungarian(ss_gumbel, repeat(ns_src), repeat(ns_tgt))
            ss_gumbel = paddle.reshape(ss_gumbel,(batch_size, gumbel_sample_num, ss_gumbel.shape[-2], ss_gumbel.shape[-1]))
            ...
            min_obj_score = obj_score.argmin(dim=1)
            ss = ss_gumbel[paddle.arange(end=batch_size), min_obj_score.indices.cpu(), :, :]
