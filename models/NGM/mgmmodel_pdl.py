...
class Net(CNN):
    ...
    def __init__(self, pretrained=True):
        ...
        for i in range(self.gnn_layer):
            if i == 0:
                gnn_layer = GNNLayer(1, 1, cfg.NGM.GNN_FEAT[i] + (1 if cfg.
                                                                  NGM.SK_EMB else 0), cfg.NGM.GNN_FEAT[i],
                                     sk_channel=cfg
                                     .NGM.SK_EMB, sk_tau=self.tau, edge_emb=cfg.NGM.EDGE_EMB)
            else:
                gnn_layer = GNNLayer(cfg.NGM.GNN_FEAT[i - 1] + (1 if cfg.
                                                                NGM.SK_EMB else 0), cfg.NGM.GNN_FEAT[i - 1], cfg.NGM.
                                     GNN_FEAT[i] + (1 if cfg.NGM.SK_EMB else 0), cfg.NGM.
        ...
        self.classifier = nn.Linear(cfg.NGM.GNN_FEAT[-1] + (1 if cfg.NGM.
                                                            SK_EMB else 0), 1)
        self.sinkhorn2 = Sinkhorn(max_iter=cfg.NGM.SK_ITER_NUM, epsilon=cfg
                                  .NGM.SK_EPSILON, tau=cfg.NGM.MGM_SK_TAU)
    ...
    def forward(self, data_dict, **kwargs):
        if 'images' in data_dict:
            ...
            data_cat = paddle.concat(data, axis=0)
            P_cat = paddle.concat(pad_tensor(Ps), axis=0)
            n_cat = paddle.concat(ns, axis=0)
            ...
            feats = paddle.concat((U, F), axis=1)
            feats = self.l2norm(feats)
            feats = paddle.split(feats, batch_size, axis=0)
        ...
        with paddle.no_grad():
            joint_S = paddle.zeros([batch_size, joint_indices[-1],
                                joint_indices[-1]])
        joint_S_diag = paddle.diagonal(joint_S, axis1=1, axis2=2)
        joint_S_diag += 1
        pred_s = []
        pred_x = []
        indices = []
        for src, tgt in combinations(feat_list, 2):
            ...
            if src_idx > tgt_idx:
                joint_S[:, joint_indices[tgt_idx]:joint_indices[tgt_idx + 1
                                                                ], joint_indices[src_idx]:joint_indices[src_idx + 1]
                ] += s.transpose((0, 2, 1))
            else:
        ...
        for b in range(batch_size):
            ...
            topargs = paddle.argsort(paddle.abs(e), descending=True)[:
                                                                     joint_indices[1]]
            diff = e[topargs[:-1]] - e[topargs[1:]]
            if paddle.min(paddle.abs(diff)) > 0.0001:
                matching_s.append(len(data) * paddle.mm(v[:, topargs], v[:,
                                                                       topargs].transpose((1,0))))
            else:
    ...
    def __ngm_forward(self, src, tgt, P_src, P_tgt, G_src, G_tgt, H_src,
                      H_tgt, K_G, K_H, ns_src, ns_tgt):
        ...
        s = paddle.reshape(v, (v.shape[0], U_tgt.shape[2], -1)).transpose((0, 2, 1))
